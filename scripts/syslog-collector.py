...
def parse_syslog(line):
    try:
        ...
        # ignora TX, MESSAGE e righe senza dati RF
        if msg_type in ('TX', 'MESSAGE'):
            return None

        crc_ok = 0 if msg_type == 'CRC' else 1

        # callsign con validazione
        ...
        if len(parts) > 8 and 'N' in parts[8]:
            try:
                lat_str = parts[8].replace('N', '').replace('S', '')
                lat = float(lat_str)
                if 'S' in parts[8]:
                    lat = -lat
            except:
        ...
        if len(parts) > 9 and ('E' in parts[9] or 'W' in parts[9]):
            try:
                lon_str = parts[9].replace('E', '').replace('W', '')
                lon = float(lon_str)
                if 'W' in parts[9]:
                    lon = -lon
            except:
...
def update_station(callsign, timestamp, distance, rssi, lat, lon, path):
    try:
        ...
        existing = db.execute('SELECT total_packets, max_distance, best_rssi, first_seen FROM stations WHERE callsign=?', (callsign,)).fetchone()
        if existing:
            total = existing[0] + 1
            max_dist = existing[1]
            max_dist_date = None
            best_rssi = existing[2]
            first_seen = existing[3]
            ...
            db.execute(
                ...
                max_distance_date=COALESCE(?,max_distance_date)
                WHERE callsign=?""",
            ...
            db.execute(
                ...
                VALUES (?,?,?,1,?,?,?,?,?,?,?)""",
                (callsign, timestamp, timestamp, max_dist, timestamp if max_dist else None,
                rssi, rssi, lat, lon, path))
        db.commit()
        db.close()
    except Exception as e:
    ...
    while True:
        ...
        for line in lines:
            ...
            db.execute(
                ...
                (ts, msg_type, callsign, path, crc_ok, rssi, snr, freq_err, distance, lat, lon, comment, line, voltage)
            )
            db.commit()
            db.close()
