...
def compute_day(date_str):
    # date_str formato YYYY-MM-DD ora di Roma
    # converti in UTC per le query
    day_start = datetime.strptime(date_str, "%Y-%m-%d")
    day_start = ROME.localize(day_start)
    ...
    db.execute("""INSERT OR REPLACE INTO daily_stats
        (date, total_packets, total_rf, total_digi, unique_stations, best_distance, best_callsign, rssi_avg, crc_errors, peak_hour)
        VALUES (?,?,?,?,?,?,?,?,?,?)""",
...
# importa dati storici dal DB
db = sqlite3.connect(DB_PATH)
dates = db.execute("""
    SELECT DISTINCT date(datetime(timestamp, '+2 hours')) as d
...
for row in dates:
    d = row[0]
    if d and d < today:  # solo giorni completi
        compute_day(d)
